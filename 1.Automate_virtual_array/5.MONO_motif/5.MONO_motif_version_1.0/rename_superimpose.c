...
int scenario=0;
int core_rn,branch_rn,Tcore_rn; //stores residue numbers of core and branch structures of the bud and the template core.
int mi,ri,ai,nb;
int c_ai,c_ri,c_mi;
int b_ai,b_ri,b_mi;
...
// redundant?
// get name of bud_core residue for renaming atoms in the complex
for(mi=0;mi<A.nm;mi++){ // for each molecule
        for(ri=0;ri<A.m[mi][0].nr;ri++){ // for each residue in molecule
                if(A.m[mi][0].r[ri].n==core_rn){// look only at core_rn residue
...
// DEBUGGING printf("ai=%d,ri=%d,.N[]=%c%c%c\n",ai,ri,B.m[mi][0].r[ri].a[ai].N[0],B.m[mi][0].r[ri].a[ai].N[1],B.m[mi][0].r[ri].a[ai].N[2]);
				...
				if(strcmp(B.m[mi][0].r[ri].a[ai].N,cplx_a4)==0){ strcpy(B.m[mi][0].r[ri].a[ai].N,"a4"); printf("\n%d\n",4);}
                        }
		}
        }
}
