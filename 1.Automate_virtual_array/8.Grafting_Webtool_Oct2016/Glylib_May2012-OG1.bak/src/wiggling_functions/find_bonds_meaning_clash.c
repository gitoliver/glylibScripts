...
for(ai=0;ai<(*A).na;ai++){// ai is now current atom to check every other atom against
		...
		//}
			...
			Anmb++;
			printf("%s%d:%s-%s%d:%s\n",(*A).m[0][0].r[ri].N,(*A).m[0][0].r[ri].n,(*A).a[ai][0].N,(*A).m[0][0].r[rii].N,(*A).m[0][0].r[rii].n,(*A).a[aii][0].N);
		}
	}
}
printf("Assembly B:\n");
for(ai=0;ai<(*B).na;ai++){// ai is now current atom to check every other atom against
		...
		//printf("ai=%d,aii=%d,ri=%d,rii=%d,dist=%.2f\n",ai,aii,ri,rii,d);
                if ( ri!=rii && d<=DIST && d>0.001){ // If not the same atom but close enough for a bond
                        Bnmb++;
			printf("%s%d:%s-%s%d:%s\n",(*B).m[0][0].r[ri].N,(*B).m[0][0].r[ri].n,(*B).a[ai][0].N,(*B).m[0][0].r[rii].N,(*B).m[0][0].r[rii].n,(*B).a[aii][0].N);
                }
        }
}
...
if (Anmb!=Bnmb){
	...
	printf("*******************************\nAre there TER cards in your library glycans? This isn't leap! Remove them!\nIf that's okay then your grafted branches may be clashing with the core, so the core is in a different conformation when bound than it likes to be in when it has this branches current conformation.\n\n");
	is_new_bond=1;
}
