...
//for(mi=nbr+1;mi<(*A).nm;mi++){
for(mi=0;mi<(*A).nm;mi++){
        for(ri=1;ri<(*A).m[mi][0].nr;ri++){ // Don't look at ri=0. This is NLN residue and overlaps there will be
                printf("Now checking overlaps for %s\n",(*A).m[mi][0].r[ri].N);
			...
			for(mii=0;mii<(*B).nm;mii++){
				for(rii=0;rii<(*B).m[mii][0].nr;rii++){
					for(aii=0;aii<(*B).m[mii][0].r[rii].na;aii++){//for current atom check dist to every other atom within the assembly B
						...
						if ( (rA + rB > d + 0.6) && (d > 0.0001) ) { // 0.6 is acceptable. 0.0001 so ignore atoms in exact same place
							swtch=1;
