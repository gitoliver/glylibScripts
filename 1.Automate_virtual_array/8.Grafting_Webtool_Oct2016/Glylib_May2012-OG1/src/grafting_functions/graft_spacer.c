...
#include <molecules.h>
#include <dirent.h>
int graft_spacer(assembly *A, assembly *B, char *fileprefix, char *id, char *linkerLib){ // A is carb. B is receptor
...
double sumOvrlap=0;
FILE *file;
printf("Grafting spacer to reducing terminal\n");
// results output
...
for(mi=0;mi<(*A).nm;mi++){ // for each molecule
    for(ri=0;ri<(*A).m[mi][0].nr;ri++){ // for each residue in molecule
        if( (strcmp((*A).m[mi][0].r[ri].N,"ROH")==0) || (strcmp((*A).m[mi][0].r[ri].N,"OME")==0) ){ // if ROH or OME
            ...
            for(ai=0;ai<(*A).m[mi][0].r[ri].na;ai++){ // for each atom in ROH/OME
                //printf("(*A).m[%d][0].r[%d].a[%d].N=%s\n",mi,ri,ai,(*A).m[mi][0].r[ri].a[ai].N);
                if( (strcmp((*A).m[mi][0].r[ri].a[ai].N,"O1")==0) || (strcmp((*A).m[mi][0].r[ri].a[ai].N,"O")==0) || (strcmp((*A).m[mi][0].r[ri].a[ai].N,"O2")==0) ){ // If O1 or O. Can be O2 if Sia
...
/* This is now done in grafting.c and passed here
...
//Read through spacer_id.txt until we find the spacer used by the ID.
...
fprintf(pFile,"Grafting %s\nSprID |  vdW  | RBP | Clashing Receptor Residues\n",spacer);
fclose (pFile);
...
//sprintf(dir, "/array1/oliver/work/8.GRAFTING/3.linkerLib/%s%c/",spacer,ano);
sprintf(dir, "%s/%s%c/",linkerLib,spacer,ano);
...
int n;
char (*d_list)[200];
d_list = malloc(10003 * sizeof *d_list);
...
// superimpose 4 atoms
    ...
    //        printf("Sp=%s,Residue=%s\n",spacer,Sp.m[mi][0].r[ri].N);
            if( (strcmp(Sp.m[mi][0].r[ri].N,spacer)==0) || (strcmp(Sp.m[mi][0].r[ri].N,"NLN")==0) || (strcmp(Sp.m[mi][0].r[ri].N,"OLS")==0) || (strcmp(Sp.m[mi][0].r[ri].N,"OLT")==0) || (strcmp(Sp.m[mi][0].r[ri].N,"S12")==0)) { // S12 is a residue in S19a.  
...
//    printf("Linker a3=%s, a4=%s\n",Sp_a3->N,Sp_a4->N);
    ...
    strcpy(pdboutname,"checknamesB.pdb");
    outputAsmblPDB(&Sp, pdboutname);
    ...
    find_vdw_clashes_spacers(&Sp,B,namelist[i]->d_name,spaceroutname, &fitterCnt, &sumRBP, &sumOvrlap);
    //free(namelist[n]);    

    sprintf(pdboutname, "%s-Sp-%s",fileprefix,namelist[i]->d_name);
    sprintf(pdboutname, "%s-%dSp.pdb",fileprefix,count); // outputs a file for each change
    printf("outputfile=%s\n",pdboutname);
    outputAsmblPDB_NoRenameResid(&Sp,pdboutname); // stopped outputting OG 11Dec2015
    count++;

}
double percent=(( ((double)fitterCnt) / (count-10000)) * 100);
pFile = fopen (spaceroutname,"a+"); // open file and append
fprintf(pFile,"TOTAL fitting spacers=%.0f\nAverage RBP=%d, Average Ovrlap=%.2f\n",percent,(sumRBP/(count-10000)),(sumOvrlap/(count-10000)));
fclose (pFile);
//free(namelist);
return 0;
}
