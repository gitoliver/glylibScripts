...
#include <mylib.h> 
#include <molecules.h>
void superimpose3atoms(assembly *A, residue *R, char *a1, char *a3, char *a4 ) { // A gets moved onto B
int mi=0,ri=0,ai=0;
coord_3D **x_listax,**x_listbx,**px_lista,**px_listb;
char out[20]; // name of output file

printf("Entered superimpose3atoms function\n");
...
px_lista=(coord_3D**)calloc((*A).na, sizeof(coord_3D*));// ptrs to co-ord of all atoms
px_listb=(coord_3D**)calloc((*R).na, sizeof(coord_3D*));
...
for(ai=0;ai<(*R).na;ai++){
    px_listb[y]=&(*R).a[ai].x;
    y++;
    if (strcmp((*R).a[ai].N,a1)==0){
        x_listbx[1]=&((*R).a[ai].x);}
    if (strcmp((*R).a[ai].N,a3)==0){
        x_listbx[3]=&((*R).a[ai].x);}
    if (strcmp((*R).a[ai].N,a4)==0){
        x_listbx[4]=&((*R).a[ai].x);}
}
...
//printf("MARK6\n");
rotate_coords_about_axis_dp_list (px_lista, (*A).na, *x_listbx[1], direction, theta3);
...
//theta=theta*-1;
rotate_coords_about_axis_dp_list (px_lista, (*A).na, *x_listbx[1], v13B, theta2); // OG16jan12
vpt_a2A = coord2_to_vec(pt,*x_listax[2]); // OG16jan12
...
//		printf("Rotating backwards by %.1f degrees\n",theta4*57.2957795);
                rotate_coords_about_axis_dp_list (px_lista, (*A).na, *x_listbx[1], v31B, theta4);
        }
	else {
//		printf("Rotating by %.1f degrees\n",theta4*57.2957795);
		rotate_coords_about_axis_dp_list (px_lista, (*A).na, *x_listbx[1], v13B, theta4);
	}
	vpt_a4A = coord2_to_vec(pt,*x_listax[4]);
	vpt_a4B = coord2_to_vec(pt,*x_listbx[4]);
	prev_theta4=theta4;
