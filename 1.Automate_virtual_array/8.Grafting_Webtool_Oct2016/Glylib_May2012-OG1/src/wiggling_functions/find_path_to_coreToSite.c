...
//*DEBUG*/                       printf("It has %d connected residues\n",(*A).m[mi][0].r[ri].nrb);
                                for(i=0;i<(*A).m[mi][0].r[ri].nrb;i++){ //for each residue connected
                                        ...
                                        // OG31Mar14 current_resid=(*A).m[mi][0].r[ri].n;
//*DEBUG*/                               printf("BACK OUT TO FIRST CALL\nResid connected to linkage=%d\n",c_ri);
						...
						branch[0]=(branch[0]+1);
						// OG31Mar14 branch[branch[0]]=(*A).m[mi][0].r[ri].n;
                                                branch[branch[0]]=ri;
                                                // OG 31Mar14found_core=0; //reset
                                        }
                                }
                        }
  //              }
...
//*DEBUG*/ printf("\nExploring ri %d which has %d neighbours\n",c_ri,nrb);

for (i=1;i<=core_ri[0];i++){ // the array size is stored in [0]
//*DBG*/	printf("cri=%d, core_ri[%d]=%d\n",cri,i,core_ri[i]);
        if (c_ri==core_ri[i]){ //FOUND_CORE!
                *found_core=1;
/*DEBUG*/       printf("Found core, resid=%d\n",core_ri[i]);
		branch[0]=((branch[0])+1);
                // OG 31Mar14 branch[branch[0]]=(*A).m[c_mi][0].r[c_ri].n;
...
//*DEBUG*/               printf("Not the same residue as previous:%d\n ... so will check it out\n",pri);
                        if (*found_core==0){
				find_branch_to_core(core_ri, cc_ri, cc_mi, c_ri, A, found_core, branch);
			}
                } else {
