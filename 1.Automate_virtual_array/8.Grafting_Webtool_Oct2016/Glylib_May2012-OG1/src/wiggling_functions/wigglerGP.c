...
while ( (total_clash>36) && (struct_total<20000) && (stop==0) ) {
	resid_clash=1000; //reset after resolved each one
	total_clash=find_vdw_clashes_return_totalGP(A,B);
	printf("total_clash=%2f\n",total_clash);
	current_resid=find_vdw_clashes_return_clashiest_resid_GP(A,B); 
	for(y=1;y<=core_ri[0];y++){ 
                printf("core_ri[%d]=%d\n",y,core_ri[y]);
		if(current_resid==core_ri[y]){
			stop=1;}} // if current_resid has already become a core then stop
	find_path_to_coreGP(A, core_ri, current_resid, branch);
	i=1;
        //printf("branch[0]=%d.branch[%d]=%d,branch[%d]=%d\n",branch[0],i,branch[i],i+1,branch[i+1]);
	while ( (resid_clash>1) && (i<branch[0]) && (stop==0) ) {
		printf("Starting wiggling\n");
		printf("branch[%d]=%d,branch[%d]=%d\n",i,branch[i],i+1,branch[i+1]);
		...
		tors=(lna-3);
                ...
                if (tors>0){
                     ...
                     generate_torsion_window_permutationsGP(wiggleWindow,A,B,tors,&struct_total,&resid_clash,current_resid,DCS,lna,linked2);
                     printf("Finished generating torsions for resid %d\n",current_resid);   
                     keep_best_structureGP(A,B,&total_clash); // otherwise move onto next linkage and see if we can fix it from there.   
                }
		i++;
                printf("i=%d,branch[0]=%d,resid_clash=%.2f,stop=%d\n",i,branch[0],resid_clash,stop);
                // Add wiggled bits to core. They won't be wiggled again.
		core_ri[0]=((core_ri[0])+1);
		core_ri[core_ri[0]]=linkage_resid;
		for (y=1;y<=core_ri[0];y++){
			printf("New core_ri[%d]=%d\n",y,core_ri[y]);
		}
	}
        printf("Out of while loop\n");
}
printf("Returning %.2f as total_clash\n",total_clash);
return total_clash;
}
