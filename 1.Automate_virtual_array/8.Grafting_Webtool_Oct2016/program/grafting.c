...
if ( argc != 3 ) {
    printf("Usage: 2.grafting.exe wiggleWindow $linkerLib \nExample: 2.grafting.exe 5 $linkerLib\n");
}
else
{
...
int wiggleWindow= atoi (argv[1]);
printf("wiggleWindow=%d\n",wiggleWindow);
char linkerLib[100];
strcpy(linkerLib,argv[2]);
...
//			printf("TnumRes=%d j=%d\n",TnumRes,j);
	...
	if(n0_budcores>0){ // Record ID of glycan as having been grafted
		n=25; //Need to start here because of ./RESULTS_library_search/ in name.
		while( (d_list[i][n] != '-') && (n < 45) ){ // Don't let ID be longer than 20 char.
    			id[n-25]=d_list[i][n];
    			n++;
		}
		id[n-25]='\0';
		...
		idfile = fopen("grafted-IDs.txt", "a+");
		fprintf(idfile,"%s\n",id);
		fclose(idfile);
	}
...
//		}
	...
	//		printf("LGcore_rn=%d,LGbranch_rn=%d\n",LGcore_rn,LGbranch_rn);
		...
		if (internal_clash==0) { // no problems
			...
			total_overlap=find_vdw_clashes_pairwize_between_Assemblies(&GR,&R,vdwoutname,core_ri,wiggleWindow); // include all branches and core
			printf("vdwoutname=%s\n",vdwoutname);
			...
			//graft_spacer(&GR, &R, d_list[i]);
 			printf("here1\n");
                        if( access( "spacer_id.txt", F_OK ) != -1 ) {
                            graft_spacer(&GR,&R,vdwoutname,id,linkerLib);
                        }
		}
		if (internal_clash==1) {
