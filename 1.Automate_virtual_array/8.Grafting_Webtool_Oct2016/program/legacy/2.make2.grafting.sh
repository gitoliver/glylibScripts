...
echo "rm -rf Structures/ Overlaps/ Spacers/ Spacer3Dfiles/ RESULTS_grafting/ RESULTS_library_search/ fittingSpacer3Dfiles/ 2>/dev/null" >> 2.grafting.sh
#echo "mkdir RESULTS_library_search/" >> 2.grafting.sh
echo "mkdir fittingSpacer3Dfiles/" >> 2.grafting.sh
...
echo "touch Spacers/tmp" >> 2.grafting.sh # grep folder for Summary, need 1 file minimum.
...
do
	...
	echo "grafting.exe $wiggleWindow $linkerLib >> grafting.log.file\$k" >> 2.grafting.sh
	...
	echo "let i+=1;" >> 2.grafting.sh
        ...
        echo "mv RESULTS_grafting/*fit* fittingSpacer3Dfiles/ 2>/dev/null" >> 2.grafting.sh	
