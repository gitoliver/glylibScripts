...
cat input.dat | sed 's/(/\[/g;
	...
	s/\ //g;' | grep -v "9Ac" | grep -v "9NAc" | grep -v "Mur" | grep -v "Lys" | grep -v "KDN" | grep -v "ola" | grep -v "6P" | grep -v "\[Gc\]" | grep -v "H2PO3" | grep -v "NH2" | grep -v "AVL" | grep -v "NKT" | grep -v "CH3" | grep -v "G-ol" | grep -v "5b1" | grep -v "6b1" > output0.txt
