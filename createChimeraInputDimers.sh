...
do
    ...
    cnt=$(($cnt+2))
    ns=$cnt
    echo "match #$ns:$4@C1,C2,O5 #0:$2@C1,C2,O5" >> input$3.cmd
    ...
    do
        ...
        tmp=$(($cnt+1))
        cp lnk$3/linker$cnt.pdb lnk$3/linker"$tmp".pdb
        ...
        echo "open lnk$3/linker"$cnt".pdb" >> input$3.cmd
        cnt=$(($cnt+1))
        echo "match #$cnt:3@C1,C2,O5 #$ns:2@C1,C2,O5" >> input$3.cmd
        echo "define plane thickness 0.1 raiseTool false color dgrey #$cnt@H" >> input$3.cmd
...
echo "Usage: ./createChimeraInputDimers.sh <dimerwithligands> <RORCR in dimer file> <grafted glycan file> <RORCR in grafted glycan file>"
echo "Example: ./createChimeraInputDimers.sh 3PHZ.pdb 302.A 270-phi-1_omega-gt.pdb_budcore_3of1.pdb 3"
echo ""
fi
