...
if (argc!=3) {
    printf("Usage: programName.exe receptorFileName ligandFileName\n");
    printf("Example:./ClashChecker.exe receptor.pdb ligand.pdb\n\n");
    printf("Note: Currently doesn't work with Hydrogen atoms in the pdb file\n");
    printf("Only works if the first character in the atom name is the element type\n");
    printf("A TOTAL of 1 is equivalent to ~1 carbon atom being buried in the protein surface\n");
    printf("Be aware the math is simplistic and \"double counts\" ligand-receptor overlaps if receptor atoms are overlapped\n");
    printf("Switch the order of the receptor and ligand files to find out which residues in each are clashing\n\n");
}
if (argc==3) {
