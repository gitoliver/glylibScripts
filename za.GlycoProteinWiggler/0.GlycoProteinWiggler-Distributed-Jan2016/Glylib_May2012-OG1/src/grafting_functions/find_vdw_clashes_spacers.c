...
int find_vdw_clashes_spacers(assembly *A, assembly *B, char *spacername, char *spaceroutname, int *fitterCnt, int *sumRBP, double *sumOvrlap){ // A is Spacer, B is receptor 
...
char filename[300], fitFileName[300]; // Name of out file
int numAtm=0;
int swtch=0;
double ovrlap_eqv; //Equivalent overlap. So can say overlap is equivalent to X occluded atoms
int recClashRi[300]; // 300 is arbitrary. [0] will be length
recClashRi[0]=0;
int i=0;
int save=1;
...
if ((ovrlap_eqv<=1.0) && (res_below==0)) {
	*fitterCnt=((*fitterCnt)+1);
        sprintf(fitFileName,"%s-fit%d.pdb",spaceroutname,*fitterCnt);
        //outputAsmblPDB_NoRenameResid(A,fitFileName);
}

*sumRBP=(*sumRBP + res_below);
*sumOvrlap=(*sumOvrlap + ovrlap_eqv);

pFile = fopen (filename,"a+");
int e=strlen(spacername);
fprintf(pFile,"%c%c%c%c%c |%6.1f | %3d | ",spacername[e-5],spacername[e-4],spacername[e-3],spacername[e-2],spacername[e-1],ovrlap_eqv,res_below);
for(i=1;i<=recClashRi[0];i++){ 
