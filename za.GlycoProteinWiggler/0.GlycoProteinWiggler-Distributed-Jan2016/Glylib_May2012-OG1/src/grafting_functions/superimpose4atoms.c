		...
		for(ai=0;ai<(*A).m[mi][0].r[ri].na;ai++){
			...
			if (strcmp((*A).m[mi][0].r[ri].a[ai].N,"a1")==0){
                        	x_listax[1]=&((*A).m[mi][0].r[ri].a[ai].x);}
                                //printf("x_listax[1].i=%f,x_listax[1].j=%f,x_listax[1].k=%f\n",(*x_listax[1]).i,(*x_listax[1]).j,(*x_listax[1]).k);}
			if (strcmp((*A).m[mi][0].r[ri].a[ai].N,"a2")==0){
                                x_listax[2]=&((*A).m[mi][0].r[ri].a[ai].x);}
				//printf("x_listax[2].i=%f,x_listax[2].j=%f,x_listax[2].k=%f\n",(*x_listax[2]).i,(*x_listax[2]).j,(*x_listax[2]).k);}
	...
	for (ri=0;ri<(*B).m[mi][0].nr;ri++){
			...
			if (strcmp((*B).m[mi][0].r[ri].a[ai].N,"a1")==0){
                        	x_listbx[1]=&((*B).m[mi][0].r[ri].a[ai].x);}
                                //printf("x_listbx[1].i=%f,x_listbx[1].j=%f,x_listbx[1].k=%f\n",(*x_listbx[1]).i,(*x_listbx[1]).j,(*x_listbx[1]).k);}
...
if (count==20){
	...
	outputAsmblPDB(B, out);
	//printf("CHECK CheckMe files! Probably rings are not same shape. The limit was %d but continuing with theta=%.1f\n",limit,theta4*57.2957795);
}
