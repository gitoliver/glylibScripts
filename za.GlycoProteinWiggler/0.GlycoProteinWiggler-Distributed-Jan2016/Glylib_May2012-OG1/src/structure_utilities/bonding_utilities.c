...
//printf("1.Checking neighbours of %s:%d:%s\n", r[0].N, r[0].n, r[0].a[iTree].N);
...
if(r[0].aT[iTree].no<=0)
    ...
    for(ni=0;ni<r[0].na;ni++)
        {
        // if(r[0].a[ni].rTi<0) OG Jan 2016. I don't know what rTi is for, but old setup sometimes skips to an atom without any incoming bonds. E.g. CME, causing the ND2 to have 2 incoming.
        if ( (r[0].a[ni].rTi<0) && (r[0].aT[ni].ni>0 ) ) //added a clause so that number of incoming bonds cannot be zero when skipping to an unseen atom. OG Jan 2016
           { 
           //printf("2.Diving to check unseen atom: %s:%d:%s\n", r[0].N, r[0].n, r[0].a[ni].N);
...
for(oi=0;oi<r[0].aT[iTree].no;oi++)
    ...
    */
    ...
    //printf("3.Diving to check neighbours of %s:%d:%s\n",r[0].N,r[0].n,r[0].a[iTree].N);
