...
br=&(*A).m[mi][0].r[linkage]; // OG 31May14
cr=&(*A).m[mi][0].r[linked2]; // OG 31May14
linked2_ri=linked2; // OG 31May14
...
//		printf("%s(ring=%c) is connected to atom %s(ring=%c)\n",(*a).N,(*a).R,(*r).a[ai].N,(*r).a[ai].R);
		...
		// If atom in ring, save any connected ring atom
                if (strcmp(na->N,"N")==0) { // So CA-CB bond will be wiggled
//                    	printf("Found %s, must be glycoprotein, dropping out\n",na->N);
                    	(*lna)=((*lna)+1);
			(*DCS).X[(*lna)+level]=&(*a).x;
