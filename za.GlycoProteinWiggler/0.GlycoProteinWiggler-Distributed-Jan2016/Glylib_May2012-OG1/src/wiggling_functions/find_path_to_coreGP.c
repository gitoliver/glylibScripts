...
void find_branch_to_coreGP(int *core_ri, int c_ri, int c_mi, int pri, assembly *A, int *found_core, int *branch);
...
/*DEBUG*/              printf("Found linkage residue (*A).m[%d][0].r[%d].n=%d\n",mi,ri,current_resid); 
...
/*DEBUG*/                       printf("It has %d connected residues\n",(*A).m[mi][0].r[ri].nrb);
                                for(i=0;i<(*A).m[mi][0].r[ri].nrb;i++){ //for each residue connected
	                                ...
	                                find_branch_to_coreGP(core_ri, c_ri, c_mi, current_resid, A, &found_core, branch); // self calling function
...
/*DBG*/                                       	printf("Resid %d leads to the core\n",(*A).m[c_mi][0].r[c_ri].n);
						...
						//branch[branch[0]]=(*A).m[mi][0].r[ri].n;
                                                found_core=0; //reset. Why reset?
                                                //Let's see if we can stop when we find the core. Not sure why I keep going.
                                                i=(*A).m[mi][0].r[ri].nrb; 
                                                printf("Just set counter to max to stop loops\n");
                                        }
                                }
                        }
                }
...
for (i=1;i<=core_ri[0];i++){ // the array size is stored in [0]
/*DBG*/	printf("cri=%d, core_ri[%d]=%d\n",cri,i,core_ri[i]);
        if (cri == (*A).m[c_mi][0].r[ core_ri[i] ].n){ //FOUND_CORE!
                *found_core=1;
/*DEBUG*/       printf("Found core, resid=%d\n",core_ri[i]);
		branch[0]=((branch[0])+1);
                branch[branch[0]]=c_ri;
...
//*DEBUG*/               printf("Not the same residue as previous:%d\n ... so will check it out\n",pri);
                        if (*found_core==0){
				find_branch_to_coreGP(core_ri, cc_ri, cc_mi, cri, A, found_core, branch);
			}
                } else {
