...
////OG2015Dec2	while ( (resid_clash>1) && (i<=branch[0]) && (stop==0) ) {
		printf("Overlap detected; allowing induced fit grafted branch\n");
//		printf("branch[%d]=%d,branch[%d+1]=%d\n",i,branch[i],i+1,branch[i+1]);
		...
		tors=(lna-3);
                ...
                if (tors>0){
                     ...
                     generate_torsion_window_permutations(wiggleWindow,A,B,tors,&struct_total,&resid_clash,current_resid,DCS,lna,linked2);
